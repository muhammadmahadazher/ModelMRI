...
This is the Contributor Covenant, version 3.0, with two things of ModelMRI's
own: a paragraph about how disagreement works in an interpretability project,
and the reporting channel this project actually has. Everything else is the
Covenant's text.
...
ModelMRI is an interpretability tool, and interpretability is a field with
genuine open disagreement. Arguing that a technique proves less than its
author claims is welcome and useful. Doing so by disparaging the person rather
than the method is not.
...
a possible violation, use GitHub's private vulnerability-reporting form for
this repository — <https://github.com/muhammadmahadazher/ModelMRI/security/advisories/new>
— with the title prefixed **`conduct:`**. It is the project's only private
channel today: only the maintainers see what is filed there, and it is not a
public issue. A dedicated address will replace this once the project has a
domain, and this paragraph will change when it does.
...
In this project the Community Moderators are the Maintainers named in
[GOVERNANCE.md](GOVERNANCE.md) — today, one person. The ladder below is the
Covenant's, and it is used as written.
...
This adaptation is, accordingly, also licensed CC BY-SA 4.0.
